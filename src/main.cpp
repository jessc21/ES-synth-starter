...
void loop()
{
  ...
  // For each row, it should set the row select address then read the columns and
  // copy the results into the inputs bitset
  // loop over 1,2,4
  for (int i = 0; i < 8; i++)
  {
    setRow(i);
  ...
  u8g2.setCursor(2, 20); // print the result on the OLED display at coordinates (2,20)
